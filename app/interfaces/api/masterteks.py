...
from app.schemas.masterteks_schema import (
    MasterTeksCreate,
    MasterTeksResponse,
    MasterTeksUpdate,
)
...
def get_masterteks_by_id(
    ...
    master_teks = uc.get_by_id(idteks)
    if not master_teks:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="MasterTeks not found",
        )
    return master_teks
...
def delete_masterteks(idteks: int, uc: MasterTeksUseCase = Depends(get_usecase)):
